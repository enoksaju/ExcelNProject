...
const INSERT_MATERIAL_SQL =
  'INSERT INTO materiales (nombre, abrev, densidad) VALUES (?,?,?)';
...
export class PeliculasService {
  private materialesObj: BehaviorSubject<IMaterial[]> = new BehaviorSubject<
    IMaterial[]
  >([]);

  private materialObj: BehaviorSubject<IMaterial> = new BehaviorSubject<
    IMaterial
  >(INITIAL_MATERIALES[0]);

  private materialConfigObj: BehaviorSubject<
    IMaterialConfig
  > = new BehaviorSubject<IMaterialConfig>({ Ancho: 0, Calibre: 0 });
  ...
  constructor(private http: HttpClient) {
    ...
    this.DB.transaction(
      tx => {
        tx.executeSql(CREATE_MATERIALES_TABLE);

        tx.executeSql('select id from materiales', [], (tx_, res) => {
          if (res.rows.length <= 0) {
            INITIAL_MATERIALES.forEach((itm, idx, arr) => {
              tx.executeSql(INSERT_MATERIAL_SQL, [
                itm.nombre,
                itm.abrev,
                itm.densidad
              ]);
        ...
        tx.executeSql(
          ...
          [localStorage.getItem('Sel_Mat_id')],
          (tx__, val) => {
  ...
  AddMaterial(obj: IMaterial) {
    this.DB.transaction(tx => {
      tx.executeSql(
        INSERT_MATERIAL_SQL,
        [obj.nombre, obj.abrev, obj.densidad],
        (tx_, result) => this.refreshCollection(tx)
      );
    });
  }
}
