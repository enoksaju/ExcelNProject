...
export class MetrosLinealesComponent implements OnInit, OnDestroy {
  ...
  constructor(private peliculasService: PeliculasService) {
    this.materialS = this.peliculasService
      .getSelectedMaterial()
      .subscribe(v => {
  ...
  ngOnInit() {
    this.materialesS = this.peliculasService
      .Materiales()
      .subscribe(mt => (this.materiales = mt));
  }
  ngOnDestroy() {
  ...
  onMaterialChange(event) {
    this.peliculasService.refreshSelectedMaterial(
      this.materiales.find(o => o.id === event.value)
    );
  }
