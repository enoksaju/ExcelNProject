...
SELECT       
	[NGAC_USERINFO].[Name] AS CLAVE, 
	CONVERT(DATETIME, CONVERT(DATE,[NGAC_AUTHLOG].[TransactionTime])) AS fecha, 
	CONVERT(FLOAT, CONVERT(DATETIME, CONVERT(TIME,[NGAC_AUTHLOG].[TransactionTime]))) AS hora
FROM ([NGAC_USERINFO] INNER JOIN
	[NGAC_AUTHLOG] ON [NGAC_USERINFO].[ID] = [NGAC_AUTHLOG].[UserID])
WHERE (NGAC_AUTHLOG.authresult = 0) 
	AND ([NGAC_AUTHLOG].[TransactionTime] BETWEEN @FechaIni AND  @FechaFin)
";
...
";
        ...
        private int progressValue
        {
            set
            {
                if (value >= this.toolStripProgressBar1.Minimum && value <= this.toolStripProgressBar1.Maximum)
                {
        ...
        private void kryptonButton1_Click(object sender, EventArgs e)
        {
                        ...
                        using (var dr = cmd.ExecuteReader())
                        {
                            ...
                            while (dr.Read())
                            {
                                for (int i = 0; i < dr.FieldCount; i++)
                                {
                                    ...
                                    var ct = countFila + 2;
                                    ExHoja.Cells.Item[ct, i + 1] = dr[i];
                                }
                                countFila += 1;

                                // Actualizo el valor del progressbar
                                progressValue = countFila;
                                var porc = ((double)countFila / (double)totalItems);
                                ExApp.StatusBar = $"*Pegando los datos de la BD del Reloj 1: {porc:P2}";
                            }
                        }
                    }
            ...
            {
                KryptonMessageBox.Show(this, ex.Message, "Algo va mal...", MessageBoxButtons.OK, MessageBoxIcon.Error);
                ExApp.StatusBar = "Error al importar datos del Reloj1...";
            }


            // Obtengo datos del Reloj2
            try
            {
                    ...
                    using (var cmd = new OleDbCommand(SQL_RELOJ_2, cnn))
                    {
                        ...
                        using (var dr = cmd.ExecuteReader())
                        {
                            while (dr.Read())
                            {
                                ...
                                var porc = (((double)countFila - (double)previusValue) / (double)totalItems);
                                ExApp.StatusBar = $"*Pegando los datos de la BD del Reloj 2: {porc:P2}";
                            }
                        }
                    }
                    cnn.Close();
