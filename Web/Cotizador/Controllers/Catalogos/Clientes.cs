	...
	public partial class CatalogosController : ApiController
	{
		...
		public async Task<IHttpActionResult> GetClientes ( int pageSize, int pageNumber, OrderTypes orderType, string orderBy, string query, bool allUsers )
		{
			try
			{
				...
				string _columnToOrder = $"{( HasProperty ( typeof ( libProduccionDataBase.Tablas.Cliente ), orderBy ) ? orderBy : "ClienteId" )} {orderType}";
				...
				List<libProduccionDataBase.Tablas.Cliente> ret = await clientesQuery.Where ( o => ( o.ClaveCliente.Contains ( query ) | o.NombreCliente.Contains ( query ) | o.NombreContacto.Contains ( query ) | ( o.Agente.Nombre + " " + o.Agente.ApellidoPaterno + " " + o.Agente.ApellidoMaterno ).Contains ( query ) ) & ( o.AgenteId == id | allUsers ) )
		...
		public async Task<IHttpActionResult> GetCliente ( int id )
		{
			...
			var cte = await ( from itm in DB.Clientes
							  where itm.Id == id
							  select new Models.FrontEndModels.Catalogos.Cliente
							  {
								  Id = itm.Id,
		...
		[HttpPost]
		[Route ( "Clientes" )]
		public async Task<IHttpActionResult> PostCliente ( Models.FrontEndModels.Catalogos.Cliente model )
		{
			// Valido el modelo
			...
			var cte = new libProduccionDataBase.Tablas.Cliente ( )
			{
		...
		[HttpPut]
		[Route ( "Clientes" )]
		public async Task<IHttpActionResult> PutCliente ( Models.FrontEndModels.Catalogos.Cliente model )
		{
			// Valido el modelo
