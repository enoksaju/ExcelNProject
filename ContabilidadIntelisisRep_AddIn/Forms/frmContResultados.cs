	...
	public partial class frmContResultados : Form {
		...
		private async void frmContResultados_Load ( object sender , EventArgs e ) {
			...
			for ( int i = config.PeriodoD; i <= config.PeriodoA; i++ ) {
				...
				var res = await db.Database.SqlQuery<Modelos.spVerContBalanza> ( $"EXEC spVerContBalanza 'gral',{config.Ejercicio },{i},{i},'{config.ConMovimientos.ToString ( )}','{config.Nivel.ToString ( )}';" ).ToListAsync ( );
